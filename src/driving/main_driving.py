...
class Drivetrain():
    ...
    def drive(self, steering_data: list) -> None:
        ...
        act_rotation_0, act_rotation_1, difference = self.myBLDC.myAS5600.rotation_difference()
        if act_rotation_0 + act_rotation_1 > 0.1:
            d_const = 0.5*difference / (0.5*(act_rotation_0 + act_rotation_1))
        else:
            d_const = 0
